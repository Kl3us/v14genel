...
module.exports = async(client, channel) => {
  ...
  if(db.fetch(`log_${channel.guild.id}`)) {
    ...
    .setDescription("#️⃣ Bir kanal oluşturuldu")
    .addFields([
              ...
              .setColor("Green")
              .setFooter({text:`${channel.guild.name}`, iconURL: `${client.user.username}`})
              .setTimestamp()
    
    channel.guild.channels.cache.get(db.fetch(`log_${channel.guild.id}`).log).send({ embeds: [embed] })
